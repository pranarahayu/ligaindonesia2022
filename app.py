...
with tab2:
    ...
    with f1:
        ...
        if (komp_filter=='Liga 1'):
            ...
            dfp_y = pct1[pct1['Team']==f_team].reset_index(drop=True)
            dfp_x = pct1[pct1['Team']!='League Average'].reset_index(drop=True)
        else:
            f_team = st.selectbox('Select Team', pd.unique(pct2_x['Team']))
            dfp_y = pct2[pct2['Team']==f_team].reset_index(drop=True)
            dfp_x = pct2[pct2['Team']!='League Average'].reset_index(drop=True)
